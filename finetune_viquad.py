...
# ==========================================
# 2. XỬ LÝ DỮ LIỆU & CACHING SQUAD FORMAT
# ==========================================
def load_viquad_data(data_path):
    examples = []
...
def convert_examples_to_features(examples, tokenizer, max_seq_length):
    ...
    for example in tqdm(examples, desc="Converting features"):
        ...
        if not example["is_impossible"] and example["answer_text"]:
...
# ==========================================
# 3. ĐÁNH GIÁ (EVALUATION - ĐƠN GIẢN HÓA)
# ==========================================
def evaluate(model, dataloader, device):
    ...
    for batch in dataloader:
        ...
        correct_exact_match += torch.sum((start_preds == start_positions) & (end_preds == end_positions)).item()
    ...
    return eval_loss, exact_match_acc
...
def train(args):
    ...
    # ---------------------------------------------
    # 4.1 XỬ LÝ CACHE CHO TRAIN / DEV / TEST
    # ---------------------------------------------
    cache_train_path = os.path.join(args.data_dir, f"cached_train_{args.max_seq_length}.pt")
    cache_dev_path = os.path.join(args.data_dir, f"cached_dev_{args.max_seq_length}.pt")
    cache_test_path = os.path.join(args.data_dir, f"cached_test_{args.max_seq_length}.pt")

    # Tập Train
    if os.path.exists(cache_train_path):
        train_features = torch.load(cache_train_path)
    else:
        train_examples = load_viquad_data(os.path.join(args.data_dir, "train.json"))
    ...
    if os.path.exists(cache_dev_path):
        ...
        dev_examples = load_viquad_data(os.path.join(args.data_dir, "dev.json"))
    ...
    if os.path.exists(cache_test_path):
        ...
        test_examples = load_viquad_data(os.path.join(args.data_dir, "test.json"))
    ...
    for epoch in range(args.epochs):
        ...
        eval_loss, eval_em = evaluate(model, dev_dataloader, device)
        
        logger.info(f"Epoch {epoch+1} - Eval Loss: {eval_loss:.4f} - Exact Match Acc: {eval_em*100:.2f}%")

        if eval_em > best_em:
            best_em = eval_em
            torch.save(model.state_dict(), best_model_path)
            logger.info(f"✨ LƯU KỶ LỤC TỐI ƯU MỚI: {best_em*100:.2f}% tại {best_model_path}")
    ...
    if test_dataloader is not None and os.path.exists(best_model_path):
        ...
        test_loss, test_em = evaluate(model, test_dataloader, device)
        logger.info(f"🎯 KẾT QUẢ TEST CUỐI CÙNG - Loss: {test_loss:.4f} - Exact Match Acc: {test_em*100:.2f}%")
    else:
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./outputs", help="Thư mục lưu output")
