...
# ==========================================
# THIẾT LẬP NHÃN CHO UIT-VSFC (SENTIMENT)
# ==========================================
LABEL_MAP = {
    "negative": 0,
    "neutral": 1,
...
def load_vsfc_sentiment_data(data_path):
    examples = []
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for i, val in enumerate(data):
            ...
            sentiment = val.get("sentiment", "").strip()
            
            if not sentence or not sentiment:
                continue
                
            if sentiment not in LABEL_MAP:
                logger.warning(f"Nhãn sentiment lạ '{sentiment}' tại index {i}. Bỏ qua.")
                continue
...
def train(args):
    ...
    cache_train = os.path.join(args.data_dir, f"cached_vsfc_sentiment_train_{args.max_seq_length}.pt")
    cache_dev = os.path.join(args.data_dir, f"cached_vsfc_sentiment_dev_{args.max_seq_length}.pt")
    cache_test = os.path.join(args.data_dir, f"cached_vsfc_sentiment_test_{args.max_seq_length}.pt")

    if os.path.exists(cache_train): train_features = torch.load(cache_train)
    else:
        train_features = convert_examples_to_features(load_vsfc_sentiment_data(os.path.join(args.data_dir, "UIT-VSFC-train.json")), tokenizer, args.max_seq_length)
        torch.save(train_features, cache_train)

    if os.path.exists(cache_dev): dev_features = torch.load(cache_dev)
    else:
        dev_features = convert_examples_to_features(load_vsfc_sentiment_data(os.path.join(args.data_dir, "UIT-VSFC-dev.json")), tokenizer, args.max_seq_length)
        torch.save(dev_features, cache_dev)

    if os.path.exists(cache_test): test_features = torch.load(cache_test)
    else:
        test_features = convert_examples_to_features(load_vsfc_sentiment_data(os.path.join(args.data_dir, "UIT-VSFC-test.json")), tokenizer, args.max_seq_length)
    ...
    best_f1 = 0.0
    best_model_path = os.path.join(args.output_dir, "best_model_vsfc_sentiment.pt")
    ...
    if test_dataloader is not None and os.path.exists(best_model_path):
        ...
        logger.info(f"🎯 KẾT QUẢ TEST SENTIMENT - Loss: {test_loss:.4f} | Acc: {test_acc*100:.2f}% | Macro F1: {test_f1*100:.2f}%")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, required=True)
    parser.add_argument("--init_checkpoint", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="./vsfc_sentiment_outputs")
