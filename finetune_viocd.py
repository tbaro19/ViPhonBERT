...
def load_viocd_data(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for key, val in data.items():
            if not val.get("review") or not val.get("label"):
                continue
            examples.append({
                "id": key,
                "text": val["review"],
                "label": LABEL_MAP[val["label"]]
            })
    return examples
...
def convert_examples_to_features(examples, tokenizer, max_seq_length):
    ...
    for example in tqdm(examples, desc="Converting features"):
        ...
        input_ids_tensor = tokenizer.encode(text) # Trả về tensor (Seq_len, 3)
...
def train(args):
    ...
    # --- XỬ LÝ CACHE ---
    cache_train_path = os.path.join(args.data_dir, f"cached_viocd_train_{args.max_seq_length}.pt")
    cache_dev_path = os.path.join(args.data_dir, f"cached_viocd_dev_{args.max_seq_length}.pt")
    ...
    if os.path.exists(cache_train_path):
        ...
        train_examples = load_viocd_data(os.path.join(args.data_dir, "train.json"))
    ...
    if os.path.exists(cache_dev_path):
        ...
        dev_examples = load_viocd_data(os.path.join(args.data_dir, "dev.json"))
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện UIT-ViOCD *****")
    best_acc = 0.0
    best_model_path = os.path.join(args.output_dir, "best_model_viocd.pt")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./viocd_outputs", help="Thư mục lưu output")
