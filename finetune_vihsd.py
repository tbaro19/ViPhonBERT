...
# ==========================================
# THIẾT LẬP 3 NHÃN CHO UIT-ViHSD
# ==========================================
LABEL_MAP = {
    "clean": 0,
    "offensive": 1,
...
def load_vihsd_data(data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        ...
        for key, val in data.items():
            if not val.get("comment") or not val.get("label"):
                continue
            
            label = val["label"]
            if label not in LABEL_MAP:
                logger.warning(f"Nhãn lạ '{label}' không có trong danh sách (clean, offensive, hate).")
                continue
...
def train(args):
    ...
    # --- XỬ LÝ CACHE ---
    cache_train_path = os.path.join(args.data_dir, f"cached_vihsd_train_{args.max_seq_length}.pt")
    cache_dev_path = os.path.join(args.data_dir, f"cached_vihsd_dev_{args.max_seq_length}.pt")
    ...
    if os.path.exists(cache_train_path):
        ...
        train_examples = load_vihsd_data(os.path.join(args.data_dir, "train.json"))
    ...
    if os.path.exists(cache_dev_path):
        ...
        dev_examples = load_vihsd_data(os.path.join(args.data_dir, "dev.json"))
    ...
    if os.path.exists(cache_test_path):
        ...
        test_examples = load_vihsd_data(os.path.join(args.data_dir, "test.json"))
    ...
    logger.info("***** Bắt đầu tiến trình Huấn luyện UIT-ViHSD *****")
    best_acc = 0.0
    best_model_path = os.path.join(args.output_dir, "best_model_vihsd.pt")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./vihsd_outputs", help="Thư mục lưu output")
