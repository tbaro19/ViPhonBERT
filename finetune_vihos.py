...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(f"Epoch {epoch+1} - Eval Loss: {eval_loss:.4f} - Token-level Acc: {eval_acc*100:.2f}%")
    ...
    if test_dataloader is not None and os.path.exists(best_model_path):
        ...
        logger.info(f"🎯 KẾT QUẢ TEST CUỐI CÙNG - Loss: {test_loss:.4f} - Token-level Acc: {test_acc*100:.2f}%")
    else:
